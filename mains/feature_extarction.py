...
def tf_idf_by_statement(statement: str, df: pd.DataFrame, export: bool):
    ...
    if export:
        tfidf_df.to_csv(f"{export_path}csv/{statement}_TFIDF.csv", index=True)

        file = open(f"{export_path}word_result_TF_IDF/{statement}_tfidf.txt", "w")
        json.dump(result_series, file)
        file.close()

        print(f"TDIDF CSV and Sum Text for '{statement}' built at {export_path}csv and {export_path}word_result_TF_IDF")


tf_idf_by_statement("true", trainingData, True)
tf_idf_by_statement("mostly-true", trainingData, True)
tf_idf_by_statement("half-true", trainingData, True)
tf_idf_by_statement("barely-true", trainingData, True)
tf_idf_by_statement("false", trainingData, True)
tf_idf_by_statement("pants-fire", trainingData, True)
