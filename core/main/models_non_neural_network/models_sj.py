...
x_train_bi, x_test_bi, x_val_bi, y_train_bi, y_test_bi, y_val_bi = f.feature_extraction_sj(c.training_set, c.testing_set, c.val_set, True)
x_train_hax, x_test_hax, x_val_hex, y_train_hax, y_test_hax, y_val_hex = f.feature_extraction_sj(c.training_set, c.testing_set, c.val_set, False)


def decision_tree():
    t1 = time.time()
    model_bi, model_hex = DecisionTreeClassifier(criterion='entropy', max_depth=100, splitter='best',
                                                 random_state=42), DecisionTreeClassifier(criterion='entropy',
                                                                                          max_depth=100,
                                                                                          splitter='best',
                                                                                          random_state=42)
...
def support_vector_machine():
    t1 = time.time()
    model_bi, model_hax = svm.SVC(kernel="linear", C=1, gamma="auto", degree=3), svm.SVC(kernel="linear", C=1,
                                                                                         gamma="auto", degree=3)
    model_bi.fit(x_train_bi, y_train_bi)
